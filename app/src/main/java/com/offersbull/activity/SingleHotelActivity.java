...
public class SingleHotelActivity extends AppCompatActivity implements ObservableScrollViewCallbacks {
    ...
    private ExpandableTextView tv_facility;
    private TextView tv_type;
    private ExpandableTextView tv_discrip;
    private LinearLayout linear_facility;
    ...
    public void init(){
        ...
        tv_facility = (ExpandableTextView) findViewById(R.id.tv_facility);
        tv_discrip = (ExpandableTextView) findViewById(R.id.tv_discrip);
        linear_facility = (LinearLayout) findViewById(R.id.linear_facility);
    ...
    private void loadRealEstateById() {
        ...
        mSubscriptions.add(NetworkUtil.getRetrofit().getHotelById(id)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(this::handleResponse,this::handleError));
    }

    private void setData(Hotel hotel, ArrayList<HotelCard> hotelCardArrayList){
        Picasso.with(this)
                .load(Constants.BASE_IMAGE_URL+hotel.getPath())
                .placeholder(R.mipmap.ic_launcher)
                .error(R.mipmap.ic_launcher)
                .into(imageView);
        tv_title.setText(hotel.getTitle());
        tv_city.setText(hotel.getCity());
        tv_name.setText("By "+hotel.getName());
        CharSequence timeAgo = DateUtils.getRelativeTimeSpanString(
                Long.parseLong(String.valueOf(getLongDate(hotel.getDate()))),
                System.currentTimeMillis(), DateUtils.SECOND_IN_MILLIS);
        tv_date.setText(timeAgo);
        tv_location.setText(hotel.getArea());
        tv_Address.setText(hotel.getAddress());
        tv_type.setText(hotel.getType());
        if (!hotel.getAmenities().isEmpty())
            tv_facility.setText(hotel.getAmenities());
        else linear_facility.setVisibility(View.GONE);

        if (!hotel.getDescription().isEmpty())
            tv_discrip.setText(hotel.getDescription());
        else linear_discription.setVisibility(View.GONE);


        HotelAdapter hotelAdapter = new HotelAdapter(this,hotelCardArrayList, Constants.SIMILAR);
    ...
    private void handleResponse(ResponseHotelSingle response) {
